...
class BysqhDb
{
     ...
     * 关联join查询
     *
     * @param $a    例如：bysqh_admin_log l ，必须要包含表前缀
     * @param $b    条件
     * @param $type 类型
     *
     * @return $this
    ...
    {
        $field       = isset($this->key['field']) ? str_replace('bysqh_', $this->config['db_prefix'],
            $this->key['field']) : ' * ';
    ...
    public function max($field = '')
    {
        ...
        $where     = ! empty($this->key['where']) ? ' WHERE '.self::getWhereArr() : '';
        $limit     = ' LIMIT 1';
        $sql       = 'SELECT MAX('.$field.') AS bysqh_max  FROM '.$this->get_tablename().$join.$where.$limit;
        $findquery = $this->execute($sql);
        if ( ! empty($findquery)) {
            $res = $findquery->fetch(PDO::FETCH_ASSOC);
            $max = ! empty($res['bysqh_max']) ? $res['bysqh_max'] : 0;
    ...
    public function min($field = '')
    {
        ...
        $where     = ! empty($this->key['where']) ? ' WHERE '.self::getWhereArr() : '';
        $limit     = ' LIMIT 1';
        $sql       = 'SELECT MIN('.$field.') AS bysqh_min  FROM '.$this->get_tablename().$join.$where.$limit;
        $findquery = $this->execute($sql);
        if ( ! empty($findquery)) {
            $res = $findquery->fetch(PDO::FETCH_ASSOC);
            $min = ! empty($res['bysqh_min']) ? $res['bysqh_min'] : 0;
    ...
    public function avg($field = '')
    {
        ...
        $where     = ! empty($this->key['where']) ? ' WHERE '.self::getWhereArr() : '';
        $limit     = ' LIMIT 1';
        $sql       = 'SELECT AVG('.$field.') AS bysqh_avg FROM '.$this->get_tablename().$join.$where.$limit;
        $findquery = $this->execute($sql);
        if ( ! empty($findquery)) {
            $res = $findquery->fetch(PDO::FETCH_ASSOC);
            $avg = ! empty($res['bysqh_avg']) ? $res['bysqh_avg'] : 0;
    ...
    public function sum($field = '')
    {
        ...
        $where     = ! empty($this->key['where']) ? ' WHERE '.self::getWhereArr() : '';
        $limit     = ' LIMIT 1';
        $sql       = 'SELECT SUM('.$field.') AS bysqh_sum FROM '.$this->get_tablename().$join.$where.$limit;
        $findquery = $this->execute($sql);
        if ( ! empty($findquery)) {
            $res = $findquery->fetch(PDO::FETCH_ASSOC);
            $sum = ! empty($res['bysqh_sum']) ? $res['bysqh_sum'] : 0;
